...
public class Q3 {
	...
	static long f(int n) {
		return (long)(0.25*Math.pow(n,5) + 0.125*Math.pow(n,4) - 0.5*Math.pow(n,3) - 0.125*Math.pow(n,2) + 0.25*n);
	}
	
	static long g(int n) {
		return (long)(0.25*Math.pow(n,5) + 0.125*Math.pow(n,4) - Math.pow(n,3) + 0.875*Math.pow(n,2) + 1.75*n);
	}
